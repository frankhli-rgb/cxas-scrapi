...
This module is currently not used in the main migration loop, and will be modernized.
We plan to add flow charts as inputs in the future.
"""
...
class CXASAsyncArtifactBuilder:
    """Handles Step 1: Production-grade asynchronous generation of migration artifacts.

    This class takes existing inputs (like tree view and BQ prod conversation data)
    and generates a series of artifacts that can be useful for downstream
    ...
    def _save_artifact(self, flow_name: str, filename: str, content: Any, is_dataframe: bool = False):
        ...
        safe_flow_name = "".join([char for char in flow_name if char.isalnum() or char in (" ", "_", "-")]).strip()
    ...
    async def _run_step_1a_inventory(self, flow_name: str, tree_view: str, context_data: Dict[str, Any]) -> str:
    ...
    async def _run_step_1b_business_logic(self, flow_name: str, inventory: str, tree_view: str, telemetry_summary: str) -> str:
        ...
        business_logic = await self.gemini.generate_async(
            prompt=prompt_1b, system_prompt=Prompts.STEP_1B_BUSINESS_LOGIC["system"]
        )
        self._save_artifact(flow_name, "1B_Business_Logic.md", business_logic)
        return business_logic

    async def _run_step_1c_requirements(self, flow_name: str, business_logic: str, tree_view: str) -> pd.DataFrame:
        """Runs Step 1C: Requirements."""
        req_instruction = CXASAsyncArtifactBuilder._get_determinism_instruction(self.determinism, "requirements")
        ...
        try:
            ...
            self._save_artifact(flow_name, "1C_Requirements.csv", df_reqs, is_dataframe=True)
        except Exception as e:
            logger.warning(f"[{flow_name}] ⚠️ Error parsing Requirements CSV: {e}")

        return df_reqs

    async def _run_step_1d_tests(self, flow_name: str, inventory: str, tree_view: str, business_logic: str, df_reqs: pd.DataFrame) -> List[Dict[str, Any]]:
        """Runs Step 1D: Tests."""
        test_instruction = CXASAsyncArtifactBuilder._get_determinism_instruction(self.determinism, "test_cases")
        reqs_context = df_reqs.to_csv(index=False) if not df_reqs.empty else "No requirements generated."
    ...
    async def run_step_1(self, flow_name: str, tree_view: str, context_data: Dict[str, Any], telemetry_summary: str) -> Dict[str, Any]:
        """Runs the full Step 1 analysis in sequence."""
        logger.info(f"[{flow_name}] Starting Async Step 1: Analysis & Logic Reconstruction")
        artifacts = {"flow_name": flow_name}

        # --- 1A: Technical Inventory ---
        artifacts["inventory"] = await self._run_step_1a_inventory(flow_name, tree_view, context_data)
        logger.info(f"[{flow_name}] ✅ 1A: Inventory Complete")

        # --- 1B: Business Logic ---
        artifacts["business_logic"] = await self._run_step_1b_business_logic(flow_name, artifacts["inventory"], tree_view, telemetry_summary)
        logger.info(f"[{flow_name}] ✅ 1B: Logic Complete")

        # --- 1C: Requirements (CSV) ---
        artifacts["requirements"] = await self._run_step_1c_requirements(flow_name, artifacts["business_logic"], tree_view)
        logger.info(f"[{flow_name}] ✅ 1C: Requirements Complete")

        # --- 1D: Test Cases (JSON) ---
        artifacts["test_cases"] = await self._run_step_1d_tests(flow_name, artifacts["inventory"], tree_view, artifacts["business_logic"], artifacts["requirements"])
