...
class SimulationEvals(Apps):
  ...
  def simulate_conversation(
    ...
    eval_conv = LLMUserConversation(
        genai_client=self.genai_client, genai_model=model, test_case=test_case
    )
      ...
      for attempt in range(max_retries):
        try:
          ...
          if attempt == max_retries - 1:
            raise e
          if console_logging:
            print(f"Warning: CXAS Agent request failed ({e}). Retrying in {2**attempt}s...")
      ...
      for output in response.outputs:
          ...
          if tool_calls_msg and hasattr(tool_calls_msg, "tool_calls"):
              for tc in tool_calls_msg.tool_calls:
                  tool_name = getattr(tc, "tool", "") or getattr(tc, "display_name", "")
    ...
    if console_logging:
      ...
      for step_prog in eval_conv.steps_progress:
          print(f"- Goal: {step_prog.step.goal} | Status: {step_prog.status.value}")
