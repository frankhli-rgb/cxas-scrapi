...
```
**NEVER call `apps.create_app()` or `cxas-eval create` during verification or eval runs.** That creates a new orphaned app. Always use the existing `deployed_app_id` from `gecx-config.json` with every SCRAPI client, `cxas-eval pull`, `cxas-eval push`, and eval script command.

## Gate 1: Pull, Lint & Push
Sync platform state to local, lint, fix issues, then push fixes back:
```bash
# 1. Pull platform state to local
GOOGLE_CLOUD_PROJECT=$PROJECT_ID .venv/bin/cxas-eval pull \
...
GOOGLE_CLOUD_PROJECT=$PROJECT_ID .venv/bin/cxas-eval push \
  --agent_dir cxas_app/ \
...
.venv/bin/python scripts/lint.py
```
The `--to` flag in `cxas-eval push` MUST use the full resource path `projects/.../apps/$APP_ID` — not just the UUID. Using the wrong path or omitting `--to` may create a new app.
...
```
The root agent MUST have `end_session` associated. Sub-agents only need it if they directly end conversations (most don't — they transfer back to root). Only add `end_session` to sub-agents when the TDD explicitly requires them to terminate calls.
...
`evals/scenarios/scenarios.yaml` (sim runner reads `meta` from here):
```yaml
meta:
  project: "<PROJECT_ID>"
  location: "<LOCATION>"
...
GOOGLE_CLOUD_PROJECT=$PROJECT_ID .venv/bin/python \
  .agents/skills/agent-foundry/scripts/scrapi-sim-runner.py run \
