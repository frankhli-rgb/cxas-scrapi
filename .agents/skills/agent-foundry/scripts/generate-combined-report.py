...
def main():
    ...
    parser.add_argument("--app-name", default=None, help="App resource name. If not provided, reads from evals/scenarios/scenarios.yaml meta.")
    ...
    # Resolve app_name from scenarios.yaml if not provided
    if not args.app_name and args.golden_run:
        scenarios_path = os.path.join(os.getcwd(), "evals", "scenarios", "scenarios.yaml")
        if os.path.exists(scenarios_path):
            with open(scenarios_path) as f:
                data = yaml.safe_load(f)
            meta = data.get("meta", {})
            args.app_name = f"projects/{meta['project']}/locations/{meta['location']}/apps/{meta['app_id']}"
        else:
            print("Error: --app-name required when evals/scenarios/scenarios.yaml not found")
            return
