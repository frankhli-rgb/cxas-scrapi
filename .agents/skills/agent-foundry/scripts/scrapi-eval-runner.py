...
EVALS_YAML = os.path.join(os.getcwd(), "evals", "scenarios", "scenarios.yaml")
GOLDEN_EVALS_DIR = os.path.join(os.getcwd(), "evals", "goldens")
REPORTS_DIR = os.path.join(os.getcwd(), "eval-reports")
...
def get_app_name(data):
    meta = data["meta"]
    return f"projects/{meta['project']}/locations/{meta['location']}/apps/{meta['app_id']}"


def get_evals_client(data):
    return Evaluations(app_name=get_app_name(data))


def get_eval_utils(data):
    return EvalUtils(app_name=get_app_name(data))
    ...
    for ev in yaml_evals:
        ...
        if expect_tools:
            scenario["scenarioExpectations"] = [
                {"toolExpectation": {"expectedToolCall": {"tool": meta["tools"][t]}}}
        ...
        if expect_criteria:
            scenario["evaluationExpectations"] = [
                meta["expectations"][c] for c in expect_criteria
            ]
