...
SIM_EVALS_YAML = os.path.join(os.getcwd(), "evals", "simulations", "simulations.yaml")
...
def load_sim_templates():
    ...
    # Index by name
    return {ev["name"]: ev for ev in data.get("evals", [])}
...
def build_test_case(ev: Dict[str, Any], meta: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
def cmd_list(args):
    ...
    data = load_yaml()
    templates = load_sim_templates()
    evals = filter_evals(data["evals"], args.priority, getattr(args, 'tag', None))
    ...
    if args.eval:
        ...
        for name, t in templates.items():
            if any(tc["name"] == name for tc in test_cases):
                continue
            tags = t.get("tags", [])
            if args.priority and args.priority.upper() not in [tg.upper() for tg in tags]:
                continue
