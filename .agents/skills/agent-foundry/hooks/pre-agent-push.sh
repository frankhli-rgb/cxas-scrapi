...
# Only act if running cxas-eval push
if echo "$cmd" | grep -qE 'cxas-eval push'; then
  ...
  if [ -d "$app_dir" ]; then
    ...
    if GOOGLE_CLOUD_PROJECT="$project" cxas-eval pull "$app_resource" --project_id "$project" --location "$location" --target_dir "$tmp_dir" 2>/dev/null; then
      # Compare platform state vs local state
      drift=$(diff -rq "$tmp_dir" "$app_dir" 2>/dev/null || true)
      if [ -n "$drift" ]; then
        msg="BLOCKED: Platform state has diverged from local files in $app_dir. Someone made changes via SCRAPI or the UI that are not in your local copy. Run 'cxas-eval pull $app_resource --project_id $project --location $location --target_dir $app_dir' to merge platform changes first, then retry the push."
        if [ "$agent" = "claude" ]; then
