...
@patch("cxas_scrapi.core.apps.types.ImportAppRequest")
@patch("cxas_scrapi.core.apps.AgentServiceClient")
@patch("google.cloud.ces_v1beta.types.ImportAppRequest.ImportOptions.ConflictResolutionStrategy")
def test_import_app_conflict_strategy(mock_strategy_enum, mock_client_cls, mock_import_app_req):
    apps_client = Apps(project_id="mock-project", location="us", creds=MagicMock())
...
@patch("cxas_scrapi.core.apps.types.ImportAppRequest")
@patch("cxas_scrapi.core.apps.AgentServiceClient")
def test_import_app_backward_compatibility(mock_client_cls, mock_import_app_req):
    apps_client = Apps(project_id="mock-project", location="us", creds=MagicMock())
    ...
    with patch("cxas_scrapi.core.apps.types.ImportAppRequest.ImportOptions") as mock_import_options:
        ...
        # We just assert it was called and populated with the Enum type correctly
...
def test_import_as_new_app(mock_client_cls, mock_import_app_req):
    ...
    apps_client.import_as_new_app(display_name="New Test App", app_content=b"content")
...
def test_export_app_local_path(mock_client_cls, mock_export_app_req, mock_open):
    ...
    mock_open.return_value.__enter__.return_value.write.assert_called_once_with(b"exported_content")
