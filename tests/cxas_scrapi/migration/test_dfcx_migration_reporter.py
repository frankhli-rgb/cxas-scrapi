...
def test_set_app_info(mock_gemini):
    ...
    assert reporter.app_info == {"source": "dfcx-123", "target_name": "cxas-app", "target_id": "cxas-456"}

def test_log_variable(mock_gemini):
    reporter = DFCXMigrationReporter(gemini_client=mock_gemini)
    reporter.log_variable("orig_var", "new_var", "STRING")
    assert len(reporter.variables) == 1
    assert reporter.variables[0] == {"original": "orig_var", "sanitized": "new_var", "type": "STRING"}
...
def test_generate_cxas_augmented_details(mock_gemini):
    ...
    assert reporter.generated_features == "Mocked User Journeys:\n1. Journey A\n2. Journey B"
