...
async def test_run_step_1a_success(builder, mock_gemini_client):
    ...
    inventory = await builder._run_step_1a_inventory("Test Flow", "Dummy Tree View", {})
...
async def test_run_step_1b_success(builder, mock_gemini_client):
    ...
    business_logic = await builder._run_step_1b_business_logic("Test Flow", "Inventory Report", "Dummy Tree View", "Telemetry Summary")
...
async def test_run_step_1c_success(builder, mock_gemini_client):
    ...
    mock_gemini_client.generate_async.return_value = "Requirement_ID,Priority,Category,Description,Expected_Behavior\nREQ1,P0,Auth,User must be authenticated,Redirect to auth page"
    
    df_reqs = await builder._run_step_1c_requirements("Test Flow", "Business Logic", "Dummy Tree View")
...
async def test_run_step_1d_success(builder, mock_gemini_client):
    ...
    tests = await builder._run_step_1d_tests("Test Flow", "Inventory Report", "Dummy Tree View", "Business Logic", pd.DataFrame())
...
async def test_run_step_1_success(builder, mock_gemini_client):
    ...
    artifacts = await builder.run_step_1("Test Flow", "Dummy Tree View", {}, "Telemetry Summary")
