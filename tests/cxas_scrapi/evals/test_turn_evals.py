...
import pandas as pd
from cxas_scrapi.evals.turn_evals import TurnEvals, TurnTestCase, TurnExpectation, TurnOperator
...
@pytest.fixture
def mock_turn_evals():
    with patch('cxas_scrapi.evals.turn_evals.Sessions'), patch('cxas_scrapi.evals.turn_evals.Variables'):
...
def test_validate_turn_test_success(mock_message_to_dict, mock_turn_evals):
                ...
                "messages": [
                    {
                        "chunks": [
                            {"text": "Hi there! I am your agent."},
                            {"toolCall": {"displayName": "product_search", "args": {"query": "shoes"}}},
                            {"toolResponse": {"displayName": "product_search", "response": {"status": "SUCCESS"}}}
                        ]
                    }
                ]
            }
    ...
    test_case = TurnTestCase(
        ...
        expectations=[
            ...
            TurnExpectation(type=TurnOperator.EQUALS, value="Hi there! I am your agent. "), # Expects trailing space due to chunk mapping loop
            TurnExpectation(type=TurnOperator.TOOL_CALLED, value="product_search"),
            TurnExpectation(type=TurnOperator.TOOL_INPUT, value={"query": "shoes"}),
            TurnExpectation(type=TurnOperator.TOOL_OUTPUT, value={"status": "SUCCESS"})
        ]
    )
...
def test_validate_turn_test_failures(mock_message_to_dict, mock_turn_evals):
                ...
                "messages": [
                    {
                        "chunks": [
                            {"text": "Nope."},
                            {"toolCall": {"displayName": "other_tool", "args": {"query": "hats"}}}
                        ]
                    }
                ]
            }
    ...
    test_case = TurnTestCase(
        ...
        expectations=[
            TurnExpectation(type=TurnOperator.CONTAINS, value="your agent"),
            TurnExpectation(type=TurnOperator.EQUALS, value="Hi there! I am your agent. "),
            TurnExpectation(type=TurnOperator.TOOL_CALLED, value="product_search"),
            TurnExpectation(type=TurnOperator.TOOL_INPUT, value={"query": "shoes"}),
...
def test_run_turn_tests(mock_message_to_dict, mock_turn_evals):
    ...
    cases = [
        TurnTestCase(
            name="t1",
            user="hi",
            expectations=[TurnExpectation(type=TurnOperator.CONTAINS, value="Hello")]
        )
    ]
